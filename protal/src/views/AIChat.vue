...
const validateFile = async (file) => {
  ...
  if (file.size > limit.maxSize) {
    return { valid: false, error: `File size cannot exceed ${limit.maxSize / 1024 / 1024}MB` }
  }
  ...
  if ((type === 'audio' || type === 'video') && limit.maxDuration) {
    try {
      ...
      if (duration > limit.maxDuration) {
        return { 
          valid: false, 
          error: `${type === 'audio' ? 'Audio' : 'Video'} duration cannot exceed${limit.maxDuration} seconds`
        }
      }
    } catch (error) {
      return { valid: false, error: 'Failed to read media duration' }
    }
  }
...
const handleFileUpload = async (event) => {
  ...
  if (totalSize > limit.maxSize * 3) { // 允许最多3个文件的总大小
    alert(`Total ${firstFileType === 'image' ? 'image' : firstFileType === 'audio' ? 'audio' : 'video'} size cannot exceed ${(limit.maxSize * 3) / 1024 / 1024}MB`)
    event.target.value = ''
    selectedFiles.value = []
    return
  }
...
const getPlaceholder = () => {
  if (selectedFiles.value.length > 0) {
    ...
    return `Selected ${selectedFiles.value.length} ${desc}`
  }
  return 'Type a message or upload files...'
}

...
const loadChat = async (chatId) => {
  ...
  try {
    ...
    const errorMessage = {
      role: 'assistant',
      content: 'Load failed. Please try again.',
      timestamp: new Date(),
      isMarkdown: true
    }
...
const handleDrop = async (event) => {
  ...
  try {
    ...
    const newChat = {
      ...
      title: `PDF Chat: ${file.name.slice(0, 20)}${file.name.length > 20 ? '...' : ''}`
    }
    ...
    currentMessages.value.push({
      role: 'assistant',
      content: `PDF uploaded: ${file.name}. You may now begin asking questions.`,
      timestamp: new Date(),
      isMarkdown: true
    })
...
const sendMessage = async () => {
  ...
  try {
    ...
    currentMessages.value[assistantMessageIndex] = {
      role: 'assistant',
      content: 'Message send failed. Please try again',
      timestamp: new Date(),
      isMarkdown: true
    }
  } finally {
...
const handleFileUpload = async (event) => {
  ...
  try {
    ...
    currentMessages.value.push({
      role: 'assistant',
      content: `PDF ${file.name} is ready for questions.`,
      timestamp: new Date(),
      isMarkdown: true
    })
